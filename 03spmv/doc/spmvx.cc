...
typedef struct {
  ...
  idx_t img_W;             /**< width of the dumped image */
  idx_t img_H;             /**< height of the dumped image */
...
struct option long_options[] = {
  ...
  {"img-W",       required_argument, 0,   0  },
  {"img-H",       required_argument, 0,   0  },
...
void usage(const char * prog) {
  ...
  fprintf(stderr,
          ...
          "  --img-W W          width of the dumped image [%ld]\n"
          "  --img-H H          height of the dumped image [%ld]\n"
    ...
    case 0:
      {
        ...
        } else if (strcmp(o, "img-W") == 0) {
          opt.img_W = atol(optarg);
        } else if (strcmp(o, "img-H") == 0) {
          opt.img_H = atol(optarg);
        } else {
          fprintf(stderr,
...
typedef struct {
  ...
  idx_t M;
  idx_t N;
  idx_t img_width;              // number of horizontal pixels (for N)
  idx_t img_height;             // number of vertial pixels (for M)
} image_t;

/**
   @brief make an img_width x img_height image for M x N matrix
 */
image_t mk_image(idx_t M, idx_t N, idx_t img_width, idx_t img_height) {
  if (img_width == 0) img_width = N;
  if (img_height == 0) img_height = M;
  double * a = (double *)xalloc(sizeof(double) * img_width * img_height);
  for (idx_t i = 0; i < img_height; i++) {
    for (idx_t j = 0; j < img_width; j++) {
      a[i * img_width + j] = 0.0;
    }
  }
  image_t img = { a, M, N, img_width, img_height };
  return img;
}
...
void image_add_pixel(image_t * img, idx_t i, idx_t j, double p) {
  idx_t img_width  = img->img_width;
  idx_t img_height = img->img_height;
  idx_t          M = img->M;
  idx_t          N = img->N;
  idx_t min_img_x = (idx_t)((     j      * img_width)  / N);
  idx_t min_img_y = (idx_t)(((M - i - 1) * img_height) / M);
  idx_t max_img_x = (idx_t)((    (j + 1) * img_width)  / N);
  idx_t max_img_y = (idx_t)(((M - i    ) * img_height) / M);
  if (min_img_x == max_img_x) max_img_x = min_img_x + 1;
  if (min_img_y == max_img_y) max_img_y = min_img_y + 1;
  assert(min_img_x < img_width);
  assert(min_img_y < img_height);
  assert(max_img_x <= img_width);
  assert(max_img_y <= img_height);
  for (idx_t y = min_img_y; y < max_img_y; y++) {
    for (idx_t x = min_img_x; x < max_img_x; x++) {
      if (0 <= x && x < img_width && 0 <= y && y < img_height) {
        (void)p;
        img->a[y * img_width + x] = 1;
      }
    }
  }
}
...
int dump_sparse_file(sparse_t A,
                     idx_t img_width, idx_t img_height, char * file) {
  printf("dumping to %s (matrix size %ld x %ld -> image size: %ld x %ld)\n",
         file, (long)A.M, (long)A.N, (long)img_width, (long)img_height);
  ...
  image_t img[1] = { mk_image(A.M, A.N, img_width, img_height) };
  for (idx_t k = 0; k < nnz; k++) {
    coo_elem_t * e = elems + k;
    row_nnz[e->i]++;
  ...
  fprintf(wp, "set multiplot\n"); /* white -> blue */

  /* non-zero matrix */
  fprintf(wp, "set lmargin screen 0.1\n");
  fprintf(wp, "set rmargin screen 0.4\n");
  fprintf(wp, "set title \"\"\n"); /* white -> blue */
  fprintf(wp, "set palette rgbformula -3,-3,2\n"); /* white -> blue */
  fprintf(wp, "$mat << EOD\n");
  idx_t k = 0;
  for (idx_t i = 0; i < img->img_height; i++) {
    for (idx_t j = 0; j < img->img_width; j++) {
  ...
  fprintf(wp, "EOD\n");
  fprintf(wp, "plot '$mat' matrix using 1:2:3 with image title \"\"\n");

  /* non-zero distribution */
  fprintf(wp, "set lmargin screen 0.6\n");
  fprintf(wp, "set rmargin screen 0.9\n");
  fprintf(wp, "$row_nnz << EOD\n");
  for (idx_t y = 0; y < M; y++) {
    idx_t i = M - 1 - y;
    fprintf(wp, "%ld %ld\n", (long)row_nnz[i], (long)y);
  }
  fprintf(wp, "EOD\n");
  fprintf(wp, "plot '$row_nnz' with lines title \"\"\n");

  fprintf(wp, "unset multiplot\n");
...
int main(int argc, char ** argv) {
  ...
  printf(" done\n"); fflush(stdout);
  if (opt.dump) {
    dump_sparse_file(A, opt.img_W, opt.img_H, opt.dump);
  }
