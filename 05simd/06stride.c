...
void loop_stride(float a, float * restrict x, float b, float * restrict y, long n) {
...
void loop_stride_v(float a, floatv * x, float b, floatv * y, long n) {
  __m512i iv = ztoL();
  asm volatile("# vloop begins");
  for (long i = 0; i < n / L; i++, iv += L) {
    floatv xi = _mm512_i32gather_ps(2 * iv, x, sizeof(float));
    y[i] = a * xi + b;
  }
...
int main(int argc, char ** argv) {
  ...
  n = (n / 16) * 16;
  float *  x = _mm_malloc(2 * n * sizeof(float), 64);
  ...
  loop_stride(a, x, b, y, n);
  loop_stride_v(a, (floatv*)x, b, (floatv*)yv, n);
  for (long i = 0; i < n; i++) {
    assert(y[i] == yv[i]);
  }
  printf("OK\n");
