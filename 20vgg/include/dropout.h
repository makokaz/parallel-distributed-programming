...
template<idx_t maxB,idx_t C,idx_t H,idx_t W>
  __global__ void curand_init_global(Dropout<maxB,C,H,W>* dev, unsigned long seed) {
...
struct Dropout {
  ...
  vec<maxB*C*H*W> rvals;        /**< temporary array for saving random values generated by curand */
#endif
...
#if __NVCC__
  /* this GPU kernel function is used to initialize the random states */
  __device__ void curand_init_dev(unsigned long *seed) {
    // Thread IDs
    ...
    assert(nthreads >= maxB*C*H*W);
    if (idx_thread >= maxB*C*H*W) return;  // Threads that are too much and not needed -> stop here
    ...
    // printf("-- thread %i state %p: seed %i, rval=%f \n", idx_thread, (void*)&states[idx_thread], *seed, rval);
  }
#endif
  /**
  ...
  */
  ...
  void set_dev(Dropout<maxB,C,H,W>* dev, bool initCurand) {
    ...
    if (initCurand) {
      /* Init curand on device */
      int num_blocks = maxB * C * H * W / 256 + 1;
      int block_sz = 256;      // Should be a multiple of 32! [Limit: 1024]
      unsigned long seed = rg.get_state();
      double t0 = cur_time();
      launch_and_sync((curand_init_global<<<num_blocks, block_sz>>>(dev, seed)));
      double t1 = cur_time();
      rg.next();

      /* Copy initialized curand array to host
        There are ::to_dev calls in other sourcefiles that would overwrite the initialized array... */
      ::to_host(&states, &dev->states, sizeof(curandState_t)*maxB*C*H*W);

#if VERBOSE
      printf("Finished curand_init@dropout with nb=%i, bs=%i in t=%f sec\n", num_blocks, block_sz, t1 - t0);
#endif
    }
#else
    (void)dev;
  ...
  */
  ...
  void forward_fast_dev(array4<maxB,C,H,W>& x) {
    ...
    y(b,c,i,j) = x(b,c,i,j) * scale;
    real rval = curand_uniform(&states[idx_thread]);
    rvals(idx_thread) = rval;
    if (rval < drop_ratio) {
      y(b,c,i,j) = 0.0;
    }
    // printf("-- thread %i state %p: rval=%f \n", idx_thread, (void*)&states[idx_thread], rval);
  }
  /**
  ...
  */
  ...
  void backward_fast_dev(array4<maxB,C,H,W>& gy) {
    ...
    const real scale = 1.0 / (1 - drop_ratio);
    gx(b,c,i,j) = scale * gy(b,c,i,j);
    real rval = rvals(idx_thread); // curand_uniform(&states[idx_thread]);
    if (rval < drop_ratio) {
      gx(b,c,i,j) = 0.0;
    }
  }
